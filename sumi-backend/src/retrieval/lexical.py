...
from src.retrieval.indexer import Document, Indexer, PgChunkTable


class PgFtsIndexer(PgChunkTable, Indexer):
    ...
    async def index(self, documents: list[Document]) -> list[tuple[str, str]]:
        """Upsert chunks. No embedding happens: this arm matches words, not vectors.

        The `tsv` column is generated by Postgres from `text` and the title in
        `metadata`, so the insert never writes it. All-or-nothing: it raises on
        failure, so the returned failed-documents list is always empty.
        """
        if not documents:
            return []
        async with (
            ...
            await cur.executemany(
                ...
                [
                    (doc.id, doc.text, doc.source, Jsonb(doc.metadata))
